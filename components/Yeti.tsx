...
const Yeti: React.FC = () => {
    ...
    useEffect(() => {
        const moveEyes = (e: any) => {
            if (svgRef.current && eyeL.current && eyeR.current) {
                const svgRect = svgRef.current.getBoundingClientRect();
                const svgWidth = svgRect.width;
                const svgHeight = svgRect.height;
    
                const xPercent = Math.min(Math.max(e.clientX - svgRect.left, 0), svgWidth) / svgWidth;
                const yPercent = Math.min(Math.max(e.clientY - svgRect.top, 0), svgHeight) / svgHeight;
    
                const minX = 60, maxX = 90; // ajustez ces valeurs
                const minY = 70, maxY = 85;
    
              // Pour l'oeil gauche
let eyeLX = minX + (maxX - minX) * xPercent;
let eyeLY = minY + (maxY - minY) * yPercent;

// Limites pour l'oeil gauche
const minEyeLX = 75; // Nouvelle limite minimale
const maxEyeLX = 125; // Nouvelle limite maximale
eyeLX = Math.min(Math.max(eyeLX, minEyeLX), maxEyeLX);

eyeL.current.cx.baseVal.value = eyeLX;
eyeL.current.cy.baseVal.value = eyeLY;

    
                // Pour l'oeil droit
const offsetX = 30; // ajustez cette valeur
let eyeRX = eyeLX + offsetX;
let eyeRY = eyeLY;

// Limites pour l'oeil droit
const minEyeRX = 115; // Nouvelle limite minimale
const maxEyeRX = 160; // limite maximale
eyeRX = Math.min(Math.max(eyeRX, minEyeRX), maxEyeRX);

eyeR.current.cx.baseVal.value = eyeRX;
eyeR.current.cy.baseVal.value = eyeRY;
  ...
  return (
    ...
    <g className="body">
        <path className="bodyBGchanged" style={{ display: "none" }}
 fill="#FFFFFF" d="M200,122h-35h-14.9V72c0-27.6-22.4-50-50-50s-50,22.4-50,50v50H35.8H0l0,91h200L200,122z"/>
...
<g className="eyeR">
 ...
 className="mouthMediumBG" d="M95,104.2c-4.5,0-8.2-3.7-8.2-8.2v-2c0-1.2,1-2.2,2.2-2.2h22c1.2,0,2.2,1,2.2,2.2v2 c0,4.5-3.7,8.2-8.2,8.2H95z"/>
        <path style={{ display: "none" }}
 className="mouthLargeBG" d="M100 110.2c-9 0-16.2-7.3-16.2-16.2 0-2.3 1.9-4.2 4.2-4.2h24c2.3 0 4.2 1.9 4.2 4.2 0 9-7.2 16.2-16.2 16.2z" fill="#617e92" stroke="#3a5e77" stroke-linejoin="round" stroke-width="2.5"/>
        <defs>
        ...
        <path clip-path="url(#mouthMask)" className="tooth" style={{ fill: "#FFFFFF" }}
